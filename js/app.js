$(document).ready(function() {
    ...
    $("a.internal").on("click", function(e) {
        ...
        function scrollToAnchor(link) {
            ...
            } else if (link.hasClass("adjusted_scroll")) {
                // destOffset = destOffset + 179;
                $("html, body").animate ({
                    scrollTop: destOffset
                }, 1000);
            }
...
//            console.log("Wciąż za blisko");
    ...
    function toggleMobileMenu() {
        ...
        var header = $("header");
        var nameOfCompany = $(".greet_message");

        mobiButton.on("click", function() {
            mobiMenu.toggle();
            if ((mobiMenu.css("display") === "block")) {
                header.css( {height: "100vh"} );
                nameOfCompany.css ( {marginTop: "0.4em"} );
            } else {
                header.css( {height: "95vh"} );
                nameOfCompany.css ( {margin: "3.4em auto 2em auto"} );
            }
        });
    }
    toggleMobileMenu();

    //Changing the smooth scroll for small screens
    function matchMediaWithListener(){
        ...
        function WidthChange(mediaQuery) {
            ...
            if(mediaQuery.matches) {
                $(".internal_dest").addClass("adjusted_scroll");
            } else {
                //Lazy line painter
                ...
                function lazyLine () {
                    ...
                    $(".lazyline").lazylinepainter({
                        ...
                        "svgData": svgToBeAnimated,
                        "strokeWidth": 3,
                        "strokeColor": "#fff",
                        "onStart": function(){ //on start of lazy painter animation
                                        $(".lazyline").on("mouseover", function(){
                                            //I'm ensuring that the stroke of the icon will change to the same color as the defined in CSS hover rule
                //                            console.log($(this));
                                             $(this).find("svg path").eq(0).attr("stroke", "#9e459a");
                                        });
                                        $(".lazyline").on("mouseout", function(){
                                            $(this).find("svg path").eq(0).attr("stroke", "#fff"); //Going back to the original white stroke color on mouseleave
                                        });
                ...
                function delayedPainter () {
                    timeoutID = window.setTimeout(lazyLine, 50);
